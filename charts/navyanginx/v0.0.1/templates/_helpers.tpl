...
*/}}
{{- define "navyanginx.nginx-certificate" }}
{{- if (not (empty .Values.ingress.nginx.certificate)) }}
{{- printf .Values.ingress.nginx.certificate }}
{{- else }}
{{- printf "%s-nginx-letsencrypt" (include "navyanginx.fullname" .) }}
{{- end }}
{{- end }}
...
*/}}
{{- define "navyanginx.nginx-hostname" }}
...
{{- else }}
{{- printf "%s-nginx" (include "navyanginx.fullname" .) }}
{{- end }}
{{- end }}
{{- end }}
...
*/}}
{{- define "navyanginx.nginx-base-url" }}
{{- if (and .Values.config.nginx.baseUrl (not (empty .Values.config.nginx.baseUrl))) }}
{{- printf .Values.config.nginx.baseUrl }}
{{- else }}
{{- if .Values.ingress.nginx.enabled }}
{{- $hostname := ((empty (include "navyanginx.nginx-hostname" .)) | ternary .Values.ingress.nginx.hostname (include "navyanginx.nginx-hostname" .)) }}
...
{{- else }}
{{- printf "http://%s" (include "navyanginx.nginx-hostname" .) }}
{{- end }}
{{- end }}
{{- end }}
